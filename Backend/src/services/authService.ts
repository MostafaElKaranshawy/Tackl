...
export default class AuthService {
    ...
    static async confirmEmail(token: string): Promise<void> {
        try {
            const decodedToken = Jwt.confirmationTokenDecode(token);

            await UserRepository.confirmUserEmail(decodedToken.id);
        } catch (error) {
            throw error;
        }
    }
        ...
        try {
            ...
            if (user.lastLinkTime) {
                ...
                if (timeSinceLastLink < oneHourInMilliseconds) {
                    throw new WrongCredentialsException("Confirmation link was sent recently. Please check your email.", 429);
                }
            }
